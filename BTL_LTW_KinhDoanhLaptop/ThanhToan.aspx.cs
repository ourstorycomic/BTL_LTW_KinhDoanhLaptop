    ...
    public partial class ThanhToan : System.Web.UI.Page
    {
        ...
        protected void rptGioHang_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (Session["GioHang"] != null)
            {
                ...
                // 2. NẾU NGƯỜI DÙNG BẤM NÚT CẬP NHẬT
                else if (e.CommandName == "CapNhat")
                {
                    // Tìm ô TextBox chứa số lượng trong hàng hiện tại
                    TextBox txtSoLuong = (TextBox)e.Item.FindControl("txtSoLuong");
                    int soLuongMoi = 1;

                    // Lấy giá trị người dùng vừa nhập
                    if (txtSoLuong != null && int.TryParse(txtSoLuong.Text, out soLuongMoi))
                    {
                        if (soLuongMoi < 1) soLuongMoi = 1; // Không cho phép nhập số lượng âm hoặc bằng 0
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            if (Convert.ToInt32(dt.Rows[i]["MaSanPham"]) == maSP)
                            {
                                ...
                                dt.Rows[i]["SoLuong"] = soLuongMoi;

                                // Tính lại thành tiền = số lượng mới * đơn giá
                                decimal donGia = Convert.ToDecimal(dt.Rows[i]["DonGia"]);
                                dt.Rows[i]["ThanhTien"] = soLuongMoi * donGia;
                                break;
                            }
                        }
                    }
                ...
                // Lưu lại Session và tải lại trang để hiển thị kết quả
                Session["GioHang"] = dt;
